  ...
  decideScreen(context) async {
    ...
    if (isLoggedIn) {
      ...
      if (userModel.username == null) {
        ...
        // home screen
        List<String> postImageList = await DatabaseFunctions.getPostImageList();
        List<String> reelVideoList = await DatabaseFunctions.getReelVideoList();
        debugPrint('postImageList: ${postImageList.length}');
        debugPrint('reelVideoList: ${reelVideoList.length}');
        Provider.of<AppData>(context, listen: false).setPostImageList =
            postImageList;
        Provider.of<AppData>(context, listen: false).setReelVideoList =
            reelVideoList;
        NavigatorFunctions.navigateAndClearStack(
